...
describe('Streams', () => {
  ...
  it('keeps a stream subscription open after catch-up and emits newly appended events', async () => {
    const streamName = 'booking-live-read';
  ...
  it('catches up from start and then continues delivering live events', async () => {
    const streamName = 'booking-live-from-start';
  ...
  it('stops delivering events after unsubscribing from a stream subscription', async () => {
    const streamName = 'booking-live-unsubscribe';
  ...
  it('delivers newly appended events to multiple subscribers on the same stream', async () => {
    const streamName = 'booking-live-multi-subscriber';

    const firstSubscription = client.subscribeToStream(streamName, {
      fromRevision: START,
    });
    const secondSubscription = client.subscribeToStream(streamName, {
      fromRevision: START,
    });
